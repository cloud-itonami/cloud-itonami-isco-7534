(ns upholcoord.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [upholcoord.store :as store]
            [upholcoord.advisor :as advisor]
            [upholcoord.governor :as governor]))
...
(def ^:private req {:upholsterer-id "upholsterer-1"})
...
(deftest hard-on-unregistered-upholsterer
  (let [st (fresh-store)
        v (governor/check {:upholsterer-id "nobody"} {} (op :log-work-record) st)]
    (is (:hard? v))
    (is (some #(= :no-upholsterer (:rule %)) (:violations v)))))
...
(deftest hard-on-scope-excluded-op-finalize-upholstery-operation
  (testing "finalizing an upholstery-execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-upholstery-operation) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-finalize-upholstery-execution-decision
  (testing "finalizing an upholstery execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-upholstery-execution-decision) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-even-with-allowlisted-op
  (testing "defense-in-depth: a rationale that itself attempts to finalize the upholstery operation is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we finalize the upholstery operation now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'staple'/'foam'/'fabric'/'upholstery'/'safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
          adv (advisor/mock-advisor)
          ops [:log-work-record :schedule-crew-operation
               :flag-safety-concern :coordinate-supply-order]]
      (doseq [o ops]
        (let [request {:upholsterer-id "upholsterer-1" :op o :workshop-id "W-1"
                        :stake :low :task "routine upholstery task"
