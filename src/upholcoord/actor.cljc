(ns upholcoord.actor
  "UpholCoordActor — the ISCO-08 7534 upholstery workshop
  ...
  resume after human sign-off. ISCO-08 7534 (Upholsterers and Related
  Workers) works with hand tools (staple guns, tack hammers, cutting
  knives) and handles padding/foam materials — a standard workshop
  hand-tool hazard — so this actor's scope stays deliberately limited
  to workshop scheduling/logistics coordination only. Modeled closely
  on cloud-itonami-isco-7319's craftcoord.actor (closest published,
  ...
  The unconditional invariant: the Upholstery Coordination Advisor
  can never directly commit an operating record the UpholCoordGovernor
  refuses, and can never make this actor finalize an
  upholstery-execution decision (e.g. deciding an upholstery job is
  ...
  makes safety-clearance decisions itself."
...
(defn build-graph
  "Build a compiled UpholCoordActor graph. `store` implements
  `upholcoord.store/Store`. `advisor` implements
  `upholcoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:upholsterer-id (:upholsterer-id request)
                                    :op (:op proposal)
